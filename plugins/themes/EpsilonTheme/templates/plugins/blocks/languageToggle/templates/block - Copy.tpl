				...
				<div class="list-group">
					...
					{foreach from=$languageToggleLocales item=localeName key=localeKey}
						<div class="locale_{$localeKey|escape}{if $localeKey == $currentLocale} current{/if}" lang="{$localeKey|escape}">
							<a class="list-group-item list-group-item-action border-start-0 border-end-0" href="{url router=$smarty.const.ROUTE_PAGE page="user" op="setLocale" path=$localeKey source=$smarty.server.REQUEST_URI}">{$localeName}</a>
						</div>
					{/foreach}
				</div>
			</div>
