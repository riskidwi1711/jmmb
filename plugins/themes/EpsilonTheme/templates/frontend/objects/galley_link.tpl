...
<a class="btn btn-sm {if $isSupplementary}obj_galley_link_supplementary btn-outline-primary{else}obj_galley_link btn-primary{/if} {$type|escape}{if $restricted} restricted{/if} galley-link animate__animated animate__fadeIn animate__delay-1s " href="{url page=$page op="view" path=$path}"{if $labelledBy} aria-labelledby={$labelledBy}{/if}>
